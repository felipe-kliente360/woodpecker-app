...
(function (global) {
  ...
  // Faixa coberta pelo banco de puzzles. Ratings reais fora dela são
  // clampados para o teto/piso pra que a busca por puzzles encontre
  // material — caso clássico: GMs com rating chess.com 2900+ caíam em
  // [3000, 3400], banco só vai até 2800, retornavam 0 puzzles.
  const PUZZLE_DB_MIN = 600;
  const PUZZLE_DB_MAX = 2800;

  // Calcula suggested_rating clampado e rating_range válido. Retorna
  // também original_rating quando difere, pra UI sinalizar o cap.
  function clampRatingPuzzle(avgRating) {
    const raw = avgRating || 1200;
    const clamped = Math.max(PUZZLE_DB_MIN, Math.min(PUZZLE_DB_MAX, raw));
    const lo = Math.max(PUZZLE_DB_MIN, clamped - 100);
    const hi = Math.min(PUZZLE_DB_MAX, clamped + 100);
    return {
      suggested_rating: clamped,
      rating_range: [lo, hi],
      original_rating: (raw === clamped) ? null : raw,
    };
  }
  ...
  function derivePuzzleProgram(tacticalProfile, avgRating, errorsByPhase) {
    ...
    const ratingClamped = clampRatingPuzzle(rating);
