...
(function (global) {
  ...
  // Lista os adversários mais enfrentados nos últimos meses, com score
  // ponderado por modalidade (rapid > blitz > bullet, mesmos pesos do
  // ratingPonderadoChesscom). Usado no modo adversario pra sugerir
  // candidatos sem o usuário precisar lembrar nicknames.
  async function topAdversarios(myUsername, opts) {
    ...
    for (const url of archives) {
      ...
      for (const g of games) {
        ...
        const wU = (g.white && g.white.username || "").toLowerCase();
        const bU = (g.black && g.black.username || "").toLowerCase();
        let oppLower = null, oppCanonical = null;
        if (wU === me && bU) { oppLower = bU; oppCanonical = g.black.username; }
        else if (bU === me && wU) { oppLower = wU; oppCanonical = g.white.username; }
        else continue;
        if (!counts[oppLower]) {
          counts[oppLower] = {
    ...
    const list = Object.values(counts).sort((a, b) => b.score - a.score);
