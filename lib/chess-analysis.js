    ...
    for (let gi = 0; gi < games.length; gi++) {
      ...
      for (const m of moves) {
        if (m.is_my_move && m.tactical_role) {
          ...
          // Usa tactical_themes (lista — fork+pin no mesmo lance contam
          // pra ambos). Reagregação final em aggregateTactical é canônica.
          for (const t of (m.tactical_themes || [])) {
            partialThemes[t] = (partialThemes[t] || 0) + 1;
          }
        }
      }
