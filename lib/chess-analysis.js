...
(function (global) {
  ...
  function initStockfish() {
    ...
    SFInitPromise = (async () => {
      ...
      // Resolve baseUrl: relativos viram absolutos (locateFile precisa absoluto
      // pra resolver wasm de dentro do Blob worker).
      let absBase = out.baseUrl;
      if (!/^https?:/.test(absBase)) {
        absBase = new URL(absBase || "", location.href).href;
      }
      const prelude =
        "self.Module = self.Module || {};" +
        "self.Module.locateFile = function(name){ return " + JSON.stringify(absBase) + " + name; };" +
