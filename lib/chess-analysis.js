...
(function (global) {
  ...
  const ENGINE_DEPTH        = 12;     // pass 2 (deep): top-1 best_move pra classificação tática
  const SHALLOW_DEPTH       = 8;      // pass 1 (scan): só cp, ~30-50ms/posição
  const MULTI_PV            = 1;      // só top-1 — classificador usa apenas bestUci
  const SKIP_OPENING_PLIES  = 12;     // primeiros plies são teoria, sem padrão treinável
  const MIN_PLIES           = 15;     // partida com menos plies é descartada (fila inteira)
  ...
  function annotateLossAndRole(moves) {
    ...
    for (let i = 0; i < moves.length; i++) {
      ...
      // Janela estendida (Modo 2 v2): além do lance imediato (i+1),
      // varre i+3 e i+5 — próximas respostas do oponente após meu
      // blunder. Captura padrões "preparatórios": eu jogo lance ruim,
      // adversário responde quieto (i+1 sem theme), mas o tema aparece
      // no segundo ou terceiro lance dele. Pula i+2 e i+4 (são meus).
      if (loss >= LOSS_BLUNDER) {
        for (let k = 1; k <= 5; k += 2) {
          const ahead = moves[i + k];
          if (!ahead) break;
          if (ahead.is_my_move) continue;
          if (ahead.tactical_theme) {
            m.tactical_theme  = ahead.tactical_theme;
            m.tactical_themes = ahead.tactical_themes;
            const lossOpp = ahead.loss_cp || 0;
            m.tactical_role = lossOpp < LOSS_BLUNDER ? 'B' : 'C';
            m.window_extended = k > 1;
            break;
          }
        }
      }
  ...
  async function analyzePlayer(username, onProgress) {
    ...
    // 3. Para cada partida: pipeline two-pass.
    //
    //    PASS 1 (shallow, depth 8): scan rápido pra obter cp[i] em todos os
    //    plies fora da abertura. ~30-50ms/posição. Custo total ~50% do
    //    pipeline antigo.
    //
    //    PASS 2 (deep, depth 12): só nos CANDIDATOS — posições onde
    //    classificação tática é necessária:
    //      · my_move com loss_provisional >= 50  → potencial Role A
    //      · opp_move imediatamente após my-blunder (loss prev >= 100)
    //        → potencial Role B/C
    //    Posições não-candidatas mantêm cp shallow (suficiente pra loss),
    //    sem theme classification (que aggregator descartaria de qualquer
    //    forma por filtro de role).
    //
    //    SKIP_OPENING_PLIES: primeiros 12 plies são teoria — best_move ≈
    //    livro, loss ≈ 0, theme vazio. Pular economiza ~15-20% do pipeline.
    const allMoves = [];
    const partialThemes = {}; // contagem incremental pra UI
    for (let gi = 0; gi < games.length; gi++) {
      ...
      onProgress({
        ...
        current: gi,
        total: games.length,
        message: 'Partida ' + (gi + 1) + '/' + games.length +
                 ' · scan rápido (' + g.time_class + ', ' + moves.length + ' lances)...'
      });
      for (let i = 0; i < moves.length; i++) {
        const m = moves[i];
        if (m.ply < SKIP_OPENING_PLIES) {
          m.cp = null;
          continue;
        }
        try {
          const eng = await analyzePosition(m.fen_before, SHALLOW_DEPTH);
          m.cp = eng.cp;
        } catch (e) {
          m.cp = null;
          console.warn("[analise] shallow falhou em ply " + i + ": " + e.message);
        }
      }

      // Loss provisional usando cps shallow. Identifica candidatos pra deep.
      const candidatos = [];
      for (let i = 0; i < moves.length; i++) {
        const m = moves[i];
        const next = moves[i + 1];
        const lossProv = (m.cp == null || !next || next.cp == null)
          ? 0 : Math.max(0, m.cp + next.cp);
        m._loss_prov = lossProv;
      }
      for (let i = 0; i < moves.length; i++) {
        const m = moves[i];
        if (m.ply < SKIP_OPENING_PLIES) continue;
        const lossProv = m._loss_prov || 0;
        const prev = moves[i - 1];
        const prevLossProv = (prev && prev.is_my_move) ? (prev._loss_prov || 0) : 0;

        const ehMyCandidato  = m.is_my_move && lossProv >= LOSS_THRESHOLD;
        const ehOppCandidato = !m.is_my_move && prev && prev.is_my_move &&
                               prevLossProv >= LOSS_BLUNDER;
        if (ehMyCandidato || ehOppCandidato) candidatos.push(i);
      }

      // PASS 2 — deep nos candidatos (best_move + classify)
      if (candidatos.length > 0) {
        onProgress({
          ...
          total: games.length,
          message: 'Partida ' + (gi + 1) + '/' + games.length +
                   ' · análise profunda (' + candidatos.length + ' posições)...'
        });
        for (const i of candidatos) {
          const m = moves[i];
          try {
            const eng = await analyzePosition(m.fen_before, ENGINE_DEPTH);
            m.cp = eng.cp;             // sobrescreve shallow com deep
            m.best_move = eng.best_move;
          } catch (e) {
            console.warn("[analise] deep falhou em ply " + i + ": " + e.message);
          }
          if (m.best_move) {
      ...
      for (const m of moves) {
        if (m.is_my_move && m.tactical_role) {
          allMoves.push(m);
          // Acumula contagem por tema pra preview realtime do wizard.
          // tactical_themes é lista de objetos { theme, confidence } —
          // extrair .theme. Reagregação final em aggregateTactical é canônica.
          for (const tobj of (m.tactical_themes || [])) {
            const tname = (tobj && tobj.theme) || (typeof tobj === 'string' ? tobj : null);
            if (!tname) continue;
            partialThemes[tname] = (partialThemes[tname] || 0) + 1;
          }
        }
      }
      // Emite snapshot acumulado pro UI ir mostrando os top temas a cada
      // partida processada — sem esperar o final.
      onProgress({
        stage: 'analyze',
        current: gi + 1,
        total: games.length,
        message: 'Partida ' + (gi + 1) + '/' + games.length + ' · concluída',
        partialThemes: Object.assign({}, partialThemes),
      });
    }
  ...
  // Lista adversários "vilões": ordenados por ameaça real (derrotas
  // ponderadas por √jogos), filtrando só quem tem ≥ minGames partidas
  // pra evitar one-offs estatísticos. Janela default 6 meses.
  // Tiebreak por score de modalidade (rapid > blitz > bullet) para
  // privilegiar partidas com mais peso competitivo.
  async function topAdversarios(myUsername, opts) {
    opts = opts || {};
    const maxMonths = opts.maxMonths || 6;
    const top = opts.top || 5;
    const minGames = opts.minGames || 3;
    if (!myUsername || !myUsername.trim()) throw new Error("Username vazio");
    const me = myUsername.trim().toLowerCase();
    const PESOS = { rapid: 2.0, blitz: 1.0, bullet: 0.5, daily: 0.0 };
    ...
    for (const url of archives) {
      ...
      const games = Array.isArray(j.games) ? j.games : [];
      for (const g of games) {
        if (!g.time_class) continue;
        const w = PESOS[g.time_class] || 0;
        if (w <= 0) continue;
        const wU = (g.white && g.white.username || "").toLowerCase();
        const bU = (g.black && g.black.username || "").toLowerCase();
        let oppLower = null, oppCanonical = null, meEhBranco = false;
        if (wU === me && bU) { oppLower = bU; oppCanonical = g.black.username; meEhBranco = true; }
        else if (bU === me && wU) { oppLower = wU; oppCanonical = g.white.username; meEhBranco = false; }
        else continue;
        if (!counts[oppLower]) {
          counts[oppLower] = {
            username: oppCanonical, total: 0,
            byTC: { rapid: 0, blitz: 0, bullet: 0 }, score: 0,
            W: 0, L: 0, D: 0,
          };
        }
        counts[oppLower].total += 1;
        if (counts[oppLower].byTC[g.time_class] !== undefined) {
          counts[oppLower].byTC[g.time_class] += 1;
        }
        counts[oppLower].score += w;
        const wRes = g.white && g.white.result;
        const bRes = g.black && g.black.result;
        let meuRes;
        if (wRes === "win")      meuRes = meEhBranco ? "W" : "L";
        else if (bRes === "win") meuRes = meEhBranco ? "L" : "W";
        else                     meuRes = "D";
        counts[oppLower][meuRes] += 1;
      }
      scanned++;
    }
    const list = Object.values(counts)
      .filter(o => o.total >= minGames)
      .map(o => Object.assign(o, { ameaca: o.L * Math.sqrt(o.total) }))
      .filter(o => o.ameaca > 0)
      .sort((a, b) => {
        if (b.ameaca !== a.ameaca) return b.ameaca - a.ameaca;
